...
For as long as I can remember, I've dreamed of creating my own content and contributing something of value back to a community that has taught me so much. My own journey, filled with endless searches for clear and meaningful information, has shown me just how vital the simple act of (clearly) explaining things is. It's an important part of how we, as humans, have always learned and grown through thousands of years: passing knowledge from one person to another, no rewards, just for the simple sake of sharing.

In my experience, many articles on machine learning can feel a bit rushed, often designed more for search engines than for deep understanding. We've all seen them: quick summaries of documentation with snippets of code, or AI-generated lists that barely scratch the surface. This is why so many of us have found a home in video content, like the brilliant [Statquest with Josh Starmer](https://www.youtube.com/@statquest), which makes complex ideas feel intuitive and engaging. Yet, for that knowledge to truly set in, we often need to go deeper. This article is my effort to create that kind of resource, that I really really hope it will bring value to whoever is reading !

I chose ***Random Forests*** for this deep dive for a few reasons. The topic itself is a perfect vehicle for exploring a wide range of core machine learning concepts. It’s a subject that frequently came up (for me) in technical interviews. There is a certain elegance to them: their logic is surprisingly intuitive, which means the wisdom behind them can be appreciated even by those without a background in computer science.

This guide is written for everyone. We will build our knowledge from the ground up, starting with the absolute basics and moving all the way to the edge of current research (or at least, the edge as of today, since the field never stops improving). The goal is for anyone to enjoy what I enjoy and see what I see.
...
Machine learning is a subfield of computer science that moves away from the paradigm of traditional programming. In a conventional program, a developer writes explicit, step-by-step instructions (the algorithm) that tell the computer exactly how to solve a problem. If you want to filter spam, you might write a rule like: `IF email_subject CONTAINS "free money" THEN mark_as_spam`. This approach is brittle and fails as soon as spammers change their tactics (and they do this a lot :D). Machine learning **inverts this relationship**. Instead of providing the rules, you provide the data (a vast collection of examples of what is and is not spam) and an algorithm that can **infer** the rules from those examples. The process of inference, which is typically a form of mathematical optimization where the algorithm adjusts its internal parameters to minimize prediction errors on the training data, is what we call "learning."

<figure class="align-center" style="width:60%">
<img src="/assets/images/Trad programming vs ML.png" alt="Traditional programming vs ML">
<figcaption>A small quick comparison between tradition programming and ML, in fact an LLM is basically the 'Rules' that are mentioned in the picture, we make a model learn how to say, token by token, a coherent speech <a href="https://www.oreilly.com/library/view/ai-and-machine/9781492078180/ch20.html"> See source here</a></figcaption>
</figure>
...
Okkay so to close this section, and to retain the most information: Random Forests is a a technique of Machine Learning (not Deep Learning because it does not involve any neural network, which is a complex form of ML) that opperates on data that is already labeled for us (supervised learning) and that can make us predict a value or a class of data.

How, in the world, does this work ? Why do we call it like that ? What is a 'Forest' in this context ? Where are the trees ? 

To answer these questions, we will step by step build the algorithm behind the method, intuitively from scratch and see all the math behind it (without being boring I promise!), the main unit of a Random Forest model is a 'Decision Tree', let's discover what it is, and why it is not sufficient alone, and why we need a lot of trees, and why they need to be 'random' and how do they communicate with each other 
...
<figcaption>Calculated gini impurity in the previous example of a good split and a bad split</figcaption>
</figure>
...
Intuitively, we know Scenario B is the better choice. But the algorithm needs a final score to compare them directly. It can't just look at the child nodes individually; it needs a single number that represents the overall "goodness" of the entire split. This is where we introduce **Gini Gain** (also called **Information Gain**).
...
To calculate it, the algorithm will perform a brute-force search. It will methodically test every possible split for every single feature. For each potential split, it will calculate the weighted average impurity of the resulting children and see how much of an improvement it is over the parent's impurity of 0.48. The split that provides the highest gain (the biggest reduction in messiness) is declared the winner and becomes the first node of our tree, and we continue like that until we build the tree !
