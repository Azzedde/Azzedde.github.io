...
To calculate it, the algorithm will perform a brute-force search. It will methodically test every possible split for every single feature. For each potential split, it will calculate the weighted average impurity of the resulting children and see how much of an improvement it is over the parent's impurity of 0.48. The split that provides the highest gain (the biggest reduction in messiness) is declared the winner and becomes the first node of our tree.


So, we've found the best first split. Are we done? Not at all.

Our simple five-person example was useful for inventing the Gini Impurity metric, but it was too clean. The first split created perfectly pure nodes (In reality, this rarely happens), so the algorithm had no more work to do. To see the full algorithm in action, we need a slightly messier, more realistic dataset.

Let's expand our bank's data to ten applicants:

| ID | Annual Income ($) | Age | Loan Amount ($) | Credit History | Defaulted |
|:---|:---|:---|:---|:---|:---|
| 1 | 35,000 | 24 | 10,000 | Good | No |
| 2 | 75,000 | 45 | 50,000 | Good | No |
| 3 | 28,000 | 29 | 15,000 | Poor | Yes |
| 4 | 110,000 | 52 | 20,000 | Excellent | No |
| 5 | 42,000 | 31 | 30,000 | Poor | Yes |
| 6 | 95,000 | 35 | 100,000 | Good | Yes |
| 7 | 60,000 | 28 | 18,000 | Good | No |
| 8 | 22,000 | 25 | 8,000 | Poor | No |
| 9 | 80,000 | 48 | 65,000 | Excellent | No |
| 10| 48,000 | 33 | 40,000 | Poor | Yes |

We now have a more challenging problem. The patterns are less obvious. Let's watch the algorithm build a tree from this data, step-by-step.
